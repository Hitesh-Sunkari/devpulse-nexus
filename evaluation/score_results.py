...
for r in results:
    ...
    reference = reference_by_id.get(rid, "")
    if isinstance(reference, dict):
        reference = reference.get("reference_answer", "")
    reference_coverage = concept_score(answer, reference) if isinstance(reference, list) else (concept_score(answer, reference.split()) if reference else 0.0)
    ...
    if hallucination:
        correctness = 0.0
    elif required_ok and coverage >= 0.75 and reference_coverage >= 0.50:
        correctness = 1.0
    elif coverage >= 0.35 or reference_coverage >= 0.35:
        correctness = 0.5
    else:
        correctness = 0.0
